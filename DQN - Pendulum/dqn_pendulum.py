...
class DDQNTrainer:
    ...
    def __init__(self, env, agent, num_episodes = 200, discrete_action_size=9, load_path=None):
    ...
    def discretize_action_space(self):
        ...
        """
        action_space = {}
        action_bin_size = (self.env.action_space.high - self.env.action_space.low) / (self.discrete_action_size - 1)
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold:
            ...
            return torch.tensor([[np.random.randint(0, self.discrete_action_size)]], 
    ...
    def dqn_train(self, reward_weight=None, plot_save_path=None, render_save_path=None, dynamic_plot=False):
        ...
        if render_save_path:
            env = gym.wrappers.Monitor(self.env, render_save_path, 
                                            video_callable=lambda x: x % (self.num_episodes//10) == 0, force=True)
        else:
            env = self.env
        ...
        for i_episode in range(self.num_episodes):
            ...
            state = torch.from_numpy(env.reset()).unsqueeze(0).to(self.device, dtype=torch.float)
            score = 0

            for t in count():
                action = self.select_action(state)
                torque = self.action_space[action.item()]
